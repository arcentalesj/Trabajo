...
<%
titulo = "CONTROL DE CITAS CON CLIENTES "
select case session("cargo")
	case "71"
		strSQL = "SELECT * FROM dbo.control_citas where idvendedor = '"+ session("usuario")+ "' order by apellido,nombre"
	case "11","02","00","04"
		strSQL = "SELECT * FROM dbo.control_citas order by idvendedor,apellido,nombre"
end select
%>
<%
Dim captados
...
<body class="cuerpo">
	...
	<table border="1" align="center">
		<tr> 
			<td width="90" class="rotulos">Cedula</font></strong></td>
			<td width="292" class="rotulos">Nombre </td>
			<td width="85" class="rotulos">Registrado el </td>
			<td width="60" class="rotulos">Cod </td>
			<td width="150" class="rotulos">Producto </td>
			<td width="70" class="rotulos">Fecha Cita </td>
		</tr>
		<tr> 
			<%While ((Repeat1__numRows <> 0) AND (NOT captados.EOF))
				...
				nombrecliente = trim(captados.Fields.Item("apellido").Value)+" "+(captados.Fields.Item("nombre").Value)
				%>
			<td  width="90" class="presentac" bgcolor="<%if (Repeat1__numRows mod 2)=0 then Response.Write (session("color")) end if%>">
				<a href="control_de_citas1.asp?<%= MM_keepBoth & MM_joinChar(MM_keepBoth) & "cedula1=" & captados.Fields.Item("cedula").Value %>">
				<%=(captados.Fields.Item("cedula").Value)%></a></td>
			<td width="292" class="presental" bgcolor="<%if (Repeat1__numRows mod 2)=0 then Response.Write (session("color")) end if%>">
				<%=(response.write(nombrecliente))%></td>
			...
			<td width="60" class="presental" bgcolor="<%if (Repeat1__numRows mod 2)=0 then Response.Write (session("color")) end if%>">
				<%=(captados.Fields.Item("idproducto").Value)%></td>
			<td width="150" class="presental" bgcolor="<%if (Repeat1__numRows mod 2)=0 then Response.Write (session("color")) end if%>">
				<%=(captados.Fields.Item("nomarticulo").Value)%></td>
...
<!--				<%'case "reg"%>
					...
					<td width="33%">
		                <FORM NAME="volver" ACTION="submenu.asp">
        		            <div align="center"><input type="submit" name="" value="Menu Seguimiento"></div>
		                </FORM>
					</td>
			<%'end select%>-->
      </tr>
